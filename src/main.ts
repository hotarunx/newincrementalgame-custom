(() => {
    ...
    ctx.resetLevel = () => {
        ...
        const bonusesReset: number[] = [4, 8, 12, 0, 1];
    ...
    function runBuyer() {
        ...
        if (rBuyerButton.classList.contains('selected')) {
            const rBuyButton: HTMLElement | null = document.querySelector('#rankreset > button');
            if (rBuyButton != null) {
                rBuyButton.click();
            }
        }
        ...
        if (lBuyerButton.classList.contains('selected')) {
            const lBuyButton: HTMLElement | null = document.querySelector('#levelreset > button');
            if (lBuyButton != null) {
                lBuyButton.click();
            }
        }
    ...
    /** 自作自動購入機のボタンを作成する */
    function createBuyerButton(label: string, buttonClass: string, index: number | null) {
        // ボタン作成
    ...
    function addMyAutoBuyerButton() {
        ...
        const autoTab = containerChildren.find(
            (element) => element.textContent != null && element.textContent.indexOf(autoTabString) >= 0
        );
